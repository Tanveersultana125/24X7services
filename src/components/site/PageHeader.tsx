          ...
          <div className={logos || collage ? "lg:order-2" : undefined}>
            ...
            {(image || logos || collage || bgImage) && stats && (
              ...
              >
                {stats.map((s) => (
                  <div key={s.label}>
                    <dt className={"font-display text-3xl tracking-tight sm:text-4xl" + (onDark ? " text-white" : "")}>{s.value}</dt>
                    <dd className={"mt-1 text-xs " + (onDark ? "text-white/65" : "text-muted")}>{s.label}</dd>
                  </div>
                ))}
              </motion.dl>
            )}
